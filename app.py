...
class SubfeatureOccurenceHashTable(HashTable):
    ...
    def increase(self, log):
        ...
        if (isinstance(subfeature_value, pd.Series)):
            subfeature_value = subfeature_value.to_list()[0]
        ...
        try:
            for subfeature in self.__subfeature_table.keys():
...
def freeman_rba_score(login_attempt, user_logs, global_logs, num_users, features=["requestXRealIP", "trackingdatauserAgent"], global_hash_table=None):
    ...
    for feature in features:
        ...
        if isinstance(feature_value, pd.Series):
            feature_value = feature_value.to_list()[0]
